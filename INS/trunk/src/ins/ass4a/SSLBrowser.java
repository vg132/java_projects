 ...
 * 
 * Can also view SSL pages.
 */
public class SSLBrowser extends JFrame implements ActionListener, KeyListener
{
	...
	private void browse()
	{
		try
		{
			...
			{
				HttpsURLConnection urls=(HttpsURLConnection)url.openConnection();
				urls.setHostnameVerifier(new HttpClientVerifier());
				br=new BufferedReader(new InputStreamReader(urls.getInputStream()));
			}
			else
			{
	...
	{
		public boolean verify(String url, SSLSession cert)
		{
			try
			{
				InetAddress iaU=InetAddress.getByName(url);
				InetAddress iaC=InetAddress.getByName(cert.getPeerHost());

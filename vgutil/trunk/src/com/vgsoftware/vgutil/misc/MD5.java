package com.vgsoftware.vgutil.misc;
 /**
	...
	* $Id: MD5.java,v 1.1.1.1 2002/10/03 13:42:25 root Exp $
	* <p>
...
public class MD5
{
	private static int buf[];	// These were originally unsigned ints.
							// This Java code makes an effort to avoid sign traps.
							// buf[] is where the hash accumulates.
	private static long bits;	// This is the count of bits hashed so far.
	private static byte in[];	// This is a buffer where we stash bytes until we have
							// enough (64) to perform a transform operation.
	private static int inint[];	// inint[] used and discarded inside transform(),
								// but why allocate it over and over?
								// (In the C version this is allocated on the stack.)

	private static void update(byte[] newbuf)
	{
		update(newbuf, 0, newbuf.length);
	}

	private static void update(byte[] newbuf, int length)
	{
		update(newbuf, 0, length);
	}

	private static void update(byte[] newbuf, int bufstart, int buflen)
	{
		int t;
		int len = buflen;
  ...
  /**
   * Get the MD5 checksum of the provided string
   *
   * @param text The text to calculate a MD5 checksum from
   *
   ...
   */
	public static String parse(String text)
	{
		...
		MD5.update(text.getBytes(),text.length());
		MD5.md5final(out);
...
*/
	private static Fcore F1 = new Fcore()
	{
	...
	private static Fcore F2 = new Fcore()
	{
		int f(int x, int y, int z)
		{
			return (y ^ (z & (x ^ y)));
	...
	private static Fcore F3 = new Fcore()
	{
		int f(int x, int y, int z)
		{
			return (x ^ y ^ z);
	...
	private static Fcore F4 = new Fcore()
	{
		int f(int x, int y, int z)
		{
			return (y ^ (x | ~z));
	...
	private static int MD5STEP(Fcore f, int w, int x, int y, int z, int data, int s)
	{
		w += f.f(x, y, z) + data;
		w = w<<s | w>>>(32-s);
		w += x;

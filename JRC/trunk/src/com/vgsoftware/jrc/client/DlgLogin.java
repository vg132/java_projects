 ...
 * a server. It returns a {@link com.vgsoftware.jrc.client.Connection} with all the
 * data set.<br/>
 * Connection data can also be saved to a XML file using the {@link com.vgsoftware.jrc.client.AddressBook}.
 */
public class DlgLogin extends JDialog implements ActionListener
{
	...
	public DlgLogin(JFrame parent)
	{
		...
		addWindowListener(new WindowAdapter()
		{
			...
			{
				((DlgLogin)e.getSource()).setVisible(false); 
			}
		});
	...
	public void actionPerformed(ActionEvent arg)
	{
		...
		else if(arg.getSource()==btnSave)
		{
			...
			ad.setPassword(txtPassword.getPassword().toString());
			ab.addAddress(ad);
		...
		else if(arg.getSource()==btnConnect)
		{
			...
			else
			{
				...
				try
				{
					...
					conn=new Connection(txtServer.getText(),Integer.parseInt(txtRemotePort.getText()),txtNick.getText(),txtPassword.getPassword().toString());
					this.setVisible(false);
				}
